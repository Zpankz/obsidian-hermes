...
export class HermesSettingsTab extends PluginSettingTab {
  ...
  display(): void {
        ...
        dropdown
          .setValue(
            this.plugin.settings?.voiceName ||
              DEFAULT_HERMES_SETTINGS.voiceName,
          )
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Define specific behaviors or rules for the assistant (added to every session)",
      )
      .addTextArea((text) => {
        text
          .setPlaceholder(
            "Define specific behaviors, personalities, or rules for the AI to follow in all interactions",
          )
    ...
    systemInstructionFragment.createSpan({
      text: "Core logic instructions for the ai assistant",
    });
    ...
    new Setting(containerEl)
      ...
      .setDesc(
        "Share your screen with the voice assistant so it can see what you are looking at",
      )
      .addToggle((toggle) => {
        toggle
          .setValue(
            this.plugin.settings?.enableVision ??
              DEFAULT_HERMES_SETTINGS.enableVision,
          )
          .onChange(async (value) => {
            if (this.plugin.settings) {
        ...
        text
          ...
          .setValue(
            this.plugin.settings?.chatHistoryFolder ||
              DEFAULT_HERMES_SETTINGS.chatHistoryFolder,
          )
          .onChange(async (value) => {
            if (this.plugin.settings) {
    ...
    // API Key Section
    new Setting(containerEl).setName("API authentication").setHeading();
    ...
    serperFragment.createSpan({
      text: "API key for image search. Get 2,500 free credits at ",
    });
    ...
    const docText = docFragment.createSpan({
      text: "API keys are handled via manual entry",
    });
    ...
    new Setting(containerEl).setDesc(docFragment);
  }
}
