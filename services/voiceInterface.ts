...
type LiveSession = {
  sendRealtimeInput: (payload: {
    media: { data: string | Uint8Array; mimeType: string };
  }) => void;
...
export class GeminiVoiceAssistant implements VoiceAssistant {
  ...
  private storedInitialState:
    | { folder: string; note: string | null }
    | undefined;
  ...
  private screenCaptureInterval: ReturnType<typeof setInterval> | null = null;
      ...
      },
      {
        ...
        onRetry: (attempt, _error) => {
          this.callbacks.onLog(
            `Connection failed, retrying attempt ${attempt}/2...`,
            "info",
          );
...
`;
      ...
      console.debug(
        `System instruction: ${systemInstruction.length} chars, folder: ${this.currentFolder}, note: ${this.currentNote}`,
      );
      ...
      this.sessionPromise = ai.live.connect({
        ...
        callbacks: {
          onopen: () => {
            ...
            this.callbacks.onLog(
              "Uplink synchronized. Voice channel active.",
              "info",
            );
            if (this.sessionPromise !== null) {
          ...
          onerror: (err: unknown) => {
            ...
            console.error(
              `[VOICE API] Error details:`,
              JSON.stringify(err, null, 2),
            );
            this.callbacks.onLog(
              `Connection error: ${getErrorMessage(err)}`,
              "error",
            );
          ...
          onclose: (event: CloseEvent) => {
            const reason =
              event.reason || String(event.code) || "Connection dropped";
            const isError = event.code !== 1000 && event.code !== 1001; // 1000=normal, 1001=going away

            if (isError) {
              console.error(
                `[VOICE API] ERROR: Connection closed abnormally: code=${event.code}, reason=${reason}`,
              );

              // Log last sent message if available
              console.error(
                `[VOICE API] Last input text was: "${this.currentInputText}"`,
              );
              console.error(
                `[VOICE API] Last output text was: "${this.currentOutputText}"`,
              );
              ...
              this.callbacks.onLog(
                `CONNECTION DROPPED: Code ${event.code} - ${reason}`,
                "error",
                undefined,
                errorDetails,
              );
      ...
      this.callbacks.onLog(
        `Link Initialization Failed: ${getErrorMessage(err)}`,
        "error",
        undefined,
        errorDetails,
      );
  ...
  private async startMicStreaming(stream: MediaStream): Promise<void> {
    ...
    workletNode.port.onmessage = (event: MessageEvent<Float32Array>) => {
      ...
      if (audioDataSize > 50000) {
        console.debug(
          `Audio streaming: ${audioDataSize} bytes, session active`,
        );
      }
      ...
      try {
        ...
        console.error(
          `Audio streaming error: ${getErrorMessage(error)}, data size: ${base64.length}`,
        );
      ...
      for (const fc of message.toolCall.functionCalls) {
        ...
        if (fc.name === "end_conversation") {
          ...
          try {
            await executeCommand(
              ...
              fc.args as ToolArgs,
              {
                ...
                onFileState: (folder, note) => {
                  ...
                  this.currentNote = Array.isArray(note)
                    ? note[note.length - 1]
                    : note;
            ...
            console.debug(
              "end_conversation executed successfully, session ending",
            );
        ...
        try {
          const response = await executeCommand(
            ...
            toolCallId,
            this.currentFolder,
          ); // Pass the toolCallId and currentFolder to executeCommand
          ...
          console.debug(
            `Tool response: ${fc.name}, size: ${responseData.length} chars`,
          );

          const responseRecord = isRecord(response) ? response : undefined;
          const responseText =
            responseRecord && typeof responseRecord.text === "string"
              ? responseRecord.text
              : undefined;
          ...
          if (!toolUpdatedMessage) {
            ...
            if (fc.name === "create_file" || fc.name === "create_directory") {
              const path =
                getStringArg(args, "filename") || getStringArg(args, "path");
              if (path) {
              ...
              const sourcePath =
                getStringArg(args, "sourcePath") ||
                getStringArg(args, "oldPath");
              const targetPath =
                getStringArg(args, "targetPath") ||
                getStringArg(args, "newPath");
            ...
            if (displayContent) {
              const groundingChunks = Array.isArray(
                responseRecord?.groundingChunks,
              )
                ? responseRecord?.groundingChunks
                : [];
              const responseFiles = Array.isArray(responseRecord?.files)
                ? responseRecord?.files.filter(
                    (file): file is string => typeof file === "string",
                  )
                : undefined;
              const responseDirectories = Array.isArray(
                responseRecord?.directories,
              )
                ? responseRecord?.directories
                    .map((dir) =>
                      isRecord(dir) && typeof dir.path === "string"
                        ? dir.path
                        : null,
                    )
              ...
              const responseFolders = Array.isArray(responseRecord?.folders)
                ? responseRecord?.folders.filter(
                    (folder): folder is string => typeof folder === "string",
                  )
                : undefined;
              const directoryInfo = Array.isArray(responseRecord?.directoryInfo)
                ? responseRecord?.directoryInfo
                : undefined;
          ...
          console.error(
            `Voice interface tool error: ${fc.name} - ${errorMessage}`,
          );
          ...
          this.callbacks.onLog(
            `Tool execution error in ${fc.name}: ${errorMessage}`,
            "error",
            undefined,
            errorDetails,
          );
          ...
          try {
            ...
            console.error(
              `Failed to send tool error response: ${getErrorMessage(error)}`,
            );
            // Don't treat this as a critical error - the session may have been intentionally closed
            // by the tool itself (e.g., end_conversation)
          }
    ...
    try {
      ...
      if (videoTrack) {
        videoTrack.addEventListener("ended", () => {
          this.callbacks.onLog("Screen capture stopped by user", "info");
      ...
      this.callbacks.onLog(
        "Screen capture active — sending frames at 0.5 FPS",
        "info",
      );

      // Send first frame immediately
      void this.sendScreenFrame();
      ...
      this.callbacks.onLog(
        `Screen capture unavailable: ${getErrorMessage(err)}`,
        "info",
      );
      // Non-fatal — voice session continues without vision
    }
  }
  ...
  private sendScreenFrame(): void {
    ...
    this.screenCtx.drawImage(video, 0, 0, width, height);

    // Extract as JPEG base64
    const dataUrl = this.screenCanvas.toDataURL("image/jpeg", 0.8);
