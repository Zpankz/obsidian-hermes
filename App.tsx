...
const App = forwardRef<AppHandle, Record<string, never>>((_, ref) => {
  ...
  const [activeSpeaker, setActiveSpeaker] = useState<"user" | "model" | "none">(
    "none",
  );
  ...
  const [hasSavedConversation, setHasSavedConversation] =
    useState<boolean>(false);
  const [voiceName, setVoiceName] = useState<string>(
    () => saved.voiceName || "Zephyr",
  );
  const [customContext, setCustomContext] = useState<string>(
    () => saved.customContext || "",
  );
  const [systemInstruction, setSystemInstruction] = useState<string>(
    () => saved.systemInstruction || DEFAULT_SYSTEM_INSTRUCTION,
  );
  ...
  const [currentNote, setCurrentNote] = useState<string | null>(
    () => saved.currentNote || null,
  );
  const [totalTokens, setTotalTokens] = useState<number>(
    () => saved.totalTokens || 0,
  );
  const [usage, setUsage] = useState<UsageMetadata>({
    totalTokenCount: saved.totalTokens || 0,
  });
  ...
  const computeDelta = (fromIndex: number): TranscriptionEntry[] => {
    return transcriptsRef.current.slice(fromIndex).filter(
      (t) =>
  ...
  const restoreConversation = (conversation?: TranscriptionEntry[]) => {
    if (conversation) {
      ...
      if (chatHistory && chatHistory.length > 0) {
        ...
        const transcriptHistory: TranscriptionEntry[] = chatHistory.map(
          (message, index) => ({
            id: `chat-${index}`,
            role: "user" as const,
            text: message,
  ...
  useEffect(() => {
    ...
    if (
      lastMsg?.role === "system" &&
      lastMsg.toolData?.name === "topic_switch"
    ) {
      console.warn(
      ...
      const transcriptsToArchive = transcripts.filter(
        (t) =>
          t.id !== "welcome-init" &&
          t.id !== lastMsg.id &&
      ...
      const chatHistoryFolder =
        currentSettings?.chatHistoryFolder || "chat-history";
      ...
      persistConversationHistory(options)
        .then((result) => {
          if (result.success) {
            ...
            addLog(
              `Persistence Failure: ${result.message}`,
              "error",
              undefined,
              errorDetails,
            );
      ...
      void (async () => {
        ...
        if (reloadedSettings) {
          ...
          setSystemInstruction(
            reloadedSettings.systemInstruction || DEFAULT_SYSTEM_INSTRUCTION,
          );
  ...
  const archiveCurrentConversation = useCallback(async () => {
    ...
    console.warn(
      "[HISTORY] EVENT: end_conversation - Session ending (archiveCurrentConversation)",
    );
  ...
  const handleSystemMessage = useCallback(
    (text: string, toolData?: ToolData) => {
      setTranscripts((prev) => {
        if (toolData?.id) {
          const existingIdx = prev.findIndex(
            (t) => t.toolData?.id === toolData.id,
          );
          if (existingIdx !== -1) {
            const next = [...prev];
            next[existingIdx] = {
  ...
  const handleImageDownload = useCallback(
    async (image: ImageSearchResult, index: number) => {
      try {
  ...
  const assistantCallbacks = useMemo(
    () => ({
      ...
      onTranscription: (
        ...
        setTranscripts((prev) => {
          const activeIdx = prev.reduceRight(
            (acc, e, i) =>
              acc !== -1 ? acc : e.role === role && !e.isComplete ? i : -1,
            -1,
          );
          if (activeIdx !== -1) {
            const updated = [...prev];
            updated[activeIdx] = {
              ...updated[activeIdx],
              text: text || updated[activeIdx].text,
              isComplete,
            };
            return updated;
          }
  ...
  useEffect(() => {
    ...
    if (activeKey && !textInterfaceRef.current) {
      ...
      textInterfaceRef.current.initialize(
        activeKey,
  ...
  const startSession = async () => {
    try {
      ...
      const conversationHistory =
        delta.length > 0 ? formatDeltaForInjection(delta) : undefined;

      if (conversationHistory) {
        addLog(
          `[CONTEXT SYNC] Including ${delta.length} messages in voice session system prompt (${conversationHistory.length} chars)`,
          "info",
        );
      }
      ...
      const errorDetails = {
        ...
        contentSize:
          voiceName.length + customContext.length + systemInstruction.length,
        stack: getErrorStack(err),
        apiCall: "startSession",
      };
      addLog(
        `Uplink Error: ${getErrorMessage(err)}`,
        "error",
        undefined,
        errorDetails,
      );
  ...
  const handleSendText = (e: React.FormEvent) => {
    ...
    if (status === ConnectionStatus.CONNECTED && assistantRef.current) {
      addLog(
        "[CONTEXT SYNC] Text input ignored - voice session active",
        "info",
      );
      return;
    }
      ...
      textInterfaceRef.current = new GeminiTextInterface({
        ...
        onUsageUpdate: (usage: UsageMetadata) => {
          ...
          if (usage.totalTokenCount !== undefined)
            setTotalTokens(usage.totalTokenCount);
        },
        onArchiveConversation: archiveCurrentConversation,
      });
    ...
    if (delta.length > 0 && textInterfaceRef.current) {
      ...
      addLog(
        `[CONTEXT SYNC] Injecting ${delta.length} messages to text interface`,
        "info",
      );
      ...
      try {
        ...
        addLog(
          "[CONTEXT SYNC] Successfully injected context to text interface",
          "info",
        );
      } catch (injectErr) {
        addLog(
          `[CONTEXT SYNC] Failed to inject context to text: ${getErrorMessage(injectErr)}`,
          "error",
        );
        console.error(
          "[CONTEXT SYNC] Text injection failed:",
          injectErr,
          "Contents were:",
          contents,
        );
  ...
  return (
    <div
      className={`hermes-root flex flex-col overflow-hidden ${isObsidianEnvironment ? "" : "standalone"}`}
    >
      {showApiKeySetup ? (
        ...
        <>
          <Settings
            ...
            onUpdateApiKey={() =>
              (
                window as { aistudio?: { openSelectKey?: () => void } }
